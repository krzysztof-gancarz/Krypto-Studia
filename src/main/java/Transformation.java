...
public class Transformation {
    ...
    public static int reverseChange(int number) {
        ...
        number=number/16;
        x=number%16;
        return reverseBox[x][y];
    }

    public static char reverseChange(char character) {
        int x,y;
        int number = (int)character;
        y=number%16;
        number=number/16;
    ...
    public static int multiply(int w, int a) {
        if (a==9) {
            w = (multiply(multiply(multiply(w, 2),2),2)^w);
        }
        else if (a==11) {
            w = (multiply(multiply(multiply(w, 2),2) ^ w,2)^w);
        }
        else if (a==13) {
            w = (multiply(multiply(multiply(w, 2) ^ w,2),2)^w);
        }
        else if (a==14) {
            w = multiply(multiply(multiply(w, 2) ^ w,2) ^ w,2);
        }
        else if (w>=0x80)
        {
            if (a==2) {
